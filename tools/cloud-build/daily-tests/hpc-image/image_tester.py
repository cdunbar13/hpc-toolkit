...
def send_deployment_to_bucket(deployment_name: str, bucket_name: str) -> None:
    ...
    with tarfile.open(outfile, "w:gz") as tar:
        tar.add(deployment_name, arcname=os.path.basename(deployment_name))
...
def create_deployment(project:str, image_name: str, zone: str,
    ...
    dep_name=f"{image_name}-{build_id}"
...
def run_tests(project: str, ramble_file: str, zones: List[str] = ["us-central1-a"],
    ...
    for img in img_names:
        ...
        for zone in zones:
            print(f"Creating deployment in zone: {zone}")
...
if __name__ == "__main__":
    ...
    run_tests(args.project, rf, args.zones, args.image_family,
